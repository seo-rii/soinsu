# Sage + cuso solver for challenge 7.  Run: sage -python solve7_cuso.py
...
e=65537; xs,ys=265,600; xl,yl=155,230
x,y=var('x y')
clear=lambda v,s,l: v & ~(((1<<l)-1)<<s)
base=clear(clear(LEAK,xs,xl),ys,yl)
def get(r,k):
    for kk in (k,str(k)):
        try:
...
for lo,hi in itertools.product(range(16),repeat=2):
    ...
    for r in cuso.find_small_roots([f],{x:(0,1<<xl),y:(0,1<<yl)},modulus='p',modulus_multiple=N,modulus_lower_bound=1<<1023):
        p = int(r['p']) if 'p' in r else C+(1<<xs)*get(r,x)+(1<<ys)*get(r,y)
        if 1 < p < N and N%p==0 and (p&MASK)==LEAK:
            q=N//p; m=pow(ct,pow(e,-1,(p-1)*(q-1)),N)
            print('p =',hex(p)); print('q =',hex(q))
            print('m.hex =',long_to_bytes(m).hex()); print('m =',long_to_bytes(m)); raise SystemExit
print('not found')
