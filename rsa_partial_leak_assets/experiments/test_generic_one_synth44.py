...
m,d=4,4
st=time.time(); hs,_=generic_shift_polys_bivar(N,c0,cy,X,Y,m=m,d=d,lll_rows=54,verbose=True); print('time',time.time()-st)
zeros=[]; minb=99999; pmb=(p**m).bit_length()
for i,h in enumerate(hs):
 v=eval_poly(h,x0,y0)
 if v==0: zeros.append(i)
 else: minb=min(minb,abs(v).bit_length())
print('zeros',zeros[:20],'count',len(zeros),'minb',minb,'pmb',pmb)
